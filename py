...
def main(argv):
  ...
  try:
      ...
      print 'bork-bot.py -p <period> -c <currency pair> -n <period of moving average'
      sys.exit(2)
 ...
 if (startTime):
          historicalDate = conn.api_query("returnChartData",{"currencyPair":pair,"start":startTime,"end":endTime,"period":period})
